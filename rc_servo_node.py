...
def clamp(x, lo, hi):  # Fonction pour saturer une valeur
    return max(lo, min(hi, x)) 


class RCServoNode(Node):  # Nœud ROS2 pour piloter le servomoteur via RC override

    def __init__(self):
        super().__init__('rc_servo_node') 

        # --- Params
        self.declare_parameter('rc_channel', 7)      # Canal RC utilisé (ici AUX7 pour le servo)
        self.declare_parameter('pwm_center', 1500)   # PWM neutre du servo (1500µs)
        self.declare_parameter('pwm_min', 1100)      # Limite basse PWM
        self.declare_parameter('pwm_max', 1900)      # Limite haute PWM
        self.declare_parameter('rate_hz', 20.0)      # Fréquence d’envoi des commandes RC
        ...
        self.get_logger().info(  # Message d’info au démarrage
            f"RCServoNode OK. Publishing /mavros/rc/override on RC{self.rc_channel} "
    ...
    def on_timer(self):
        ...
        if 0 <= idx < 18:
            channels[idx] = pwm  # Écriture du PWM uniquement sur le canal du servo
